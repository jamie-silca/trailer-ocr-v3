"""
EXP-30 Qwen3-VL horizontal fallback (OpenRouter) — production module.

Mirror of app/qwen_portrait.py with two differences for horizontal trailer
plates:
  * Universal-orientation prompt (no top-to-bottom bias).
  * No format whitelist gate — accepts the model's raw answer
    (uppercased, whitespace-stripped). Wide / very_wide reads include
    container codes (CXTU…), R-prefix, plain numerics, etc., and a
    JBHZ-style format whitelist would reject most of them.

Used as a CASCADE fallback in app/ocr_processor.py — fires only when:
  1. PaddleOCR + cascade-retry returned no text, AND
  2. The crop is horizontal (image.width >= image.height).

Quantitative basis (EXP-30, dataset 20260423): on the 7 horizontal
crops where paddle returned no-text, Qwen rescued 3 (L906×2, 77233) and
introduced 4 wrong-text reads. Net +3 EXACT, ~-2 pp precision.
"""
...
class QwenHorizontalProcessor:
    ...
    def _initialize(self, model_id: str):
        ...
        cache_env = os.environ.get("QWEN_HORIZONTAL_CACHE_DIR") or os.environ.get("QWEN_CACHE_DIR")
        if cache_env:
        ...
        logger.info(
            f"QwenHorizontalProcessor ready: model={model_id}, "
    ...
    def _cache_put(self, key: str, record: dict) -> None:
        ...
        try:
            ...
            logger.warning(f"Qwen horizontal cache write failed: {e}")
        ...
        for attempt in range(self.MAX_RETRIES):
            try:
                ...
                if r.status_code != 200:
                    ...
                    logger.warning(f"Qwen horizontal attempt {attempt + 1} failed: {last_err}")
                ...
                if attempt < self.MAX_RETRIES - 1:
                    time.sleep(self.RETRY_BACKOFF_S * (attempt + 1))
        raise RuntimeError(
            f"Qwen horizontal call failed after {self.MAX_RETRIES} attempts: {last_err}"
        )
