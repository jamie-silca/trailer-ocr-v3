"""
EXP-25 — Qwen3-VL fallback for portrait-bucket crops via OpenRouter.

Same ``process_image(pil) -> (text, conf)`` contract as OcrProcessor /
VlmPortraitProcessor, with an on-disk PNG-hash cache so re-runs of the
benchmark don't re-bill the API.

Used as a CASCADE fallback in tests/benchmark_ocr.py — the standard PaddleOCR
pipeline runs first; this is only invoked when PaddleOCR returns no text on
a portrait crop.
...
# Identical wording to tests/vlm_portrait.py (EXP-18) so spike-vs-benchmark
# numbers compare apples-to-apples. PROMPT_VERSION bumps if the wording changes.
PROMPT = (
...
MIN_LONG_SIDE_PX = 768  # Match EXP-18 + spike: VLM tile pipelines need usable detail.

FORMAT_RE = re.compile(r"^(JBHZ\d{6}|JBHU\d{6}|R\d{5})$")
UNKNOWN_SENTINEL = "UNKNOWN"

CACHE_DIR = Path(__file__).parent / "results" / "qwen_cache"
...
class QwenPortraitProcessor:
    ...
    def _initialize(self, model_id: str):
        ...
        logger.info(f"Initialising QwenPortraitProcessor ({model_id})...")
        ...
        logger.info(f"QwenPortraitProcessor ready: model={model_id}, cache={CACHE_DIR}")
